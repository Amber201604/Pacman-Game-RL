...
##            print()
    ...
    """
    def max_with_broken_ties(self, array):
        maxes = []
        best = -np.inf
    ...
    """
    def act(self, action_num, value_table):
    ...
    def eps_greedy(self, table, cookie, current_position):
        if KINGS:
            moves = list(range(8))
        else:
            moves = list(range(4))
    ...
    """
    def graph_reward_over_time(self, rewards):
        plt.plot(rewards)
    ...
    """
    def graph_two_rewards_over_time(self, q_rewards, sarsa_rewards):
    ...
    def run_q_simulation(self, num_episodes):
        global current_position
        global collide_flag
    ...
    ##            if state[0] == 28 and state[1] == 5 and state not in tmp_state:
    ##                tmp_state.append(state)
    ##            elif state[0] == 12 and state[1] == 20 and state not in tmp_state:
    ##                tmp_state.append(state)
    ##                
    ##            elif state[0] == 28 and state[1] == 20 and len(tmp_state) == 2:
    ##                reach_goal = True
                if state[0] == 28 and state[1] == 20:
                    reach_goal = True
