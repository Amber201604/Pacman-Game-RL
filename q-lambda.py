...
def act(action_num, value_table):
    ...
    current_position = [ # - because 0,0 is top left
    current_position[0] + action[0],
    current_position[1] + action[1]
    ]

    # Verify validity of value, and if valid set as new value
    if current_position[0] < 0 or current_position[0] > bounds[0] or current_position[1] < 0 or current_position[1] > bounds[1] or value_table[current_position[0], current_position[1]] == 0:
...
def q_update(sars):
    ...
    #print(q_table[state_0, state_1, cookie])
...
def run_q_simulation(num_episodes, display_graph=False):
    ...
    for ep in range(num_episodes):
        ...
        action = eps_greedy(table=q_lambda_table)
        ...
        while cookies > 0:
            ...
            # if collide wall, stay at the original position and not update q_table
            # if collide_flag:                      
            discounted_reward += reward

            # Choose A' from S' using policy derived from Q (e.g., "epsilon-greedy)
            next_action = eps_greedy(q_table, cookie, current_position)
...
##                if cookies == 0:
##                    print(state)
##                    print(q_table[state[0], state[1], cookie])
##                    print(cookie)
                value_table[current_position[0], current_position[1]] = 2
                next_state = current_position[0], current_position[1]
                # a* <- argmax Q(s',b) (if a' ties for the max, then a* <- a')
                action_max = max(q_lambda_table[next_state[0], next_state[1], cookie])
                if action_max != q_lambda_table[next_state[0], next_state[1], cookie, next_action]:
                    action_next_value = action_max
                    loop_bool = False
                else:
                    action_next_value = q_lambda_table[next_state[0], next_state[1], next_action]
                    loop_bool = True

                target = reward + gamma*action_next_value - q_lambda_table[state[0], state[1], action]
                e[state[0], state[1], action] += 1

                for s0 in range(7):
                for s1 in range(10):
                if s0 == goal[0] and s1 == goal[1]:
                continue
                for action in range(num_actions):
                q_lambda_table[s0, s1, action] = q_lambda_table[s0, s1, action] + alpha*target*e[s0, s1, action] 
                if loop_bool:
                e[s0, s1, action] = gamma*lam*e[s0, s1, action]
                else:
                e[s0, s1, action] = 0
...
run_q_simulation(6000, display_graph=True)
