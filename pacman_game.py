...
##q = Test
##Q_table = q.run_q_simulation(500, display_graph=True)
## Costanti
FRAME_RATE = 30
...
while playing:
    ...
    # table position:
##    if round((pacman._x - 8) / 8) == ((pacman._x - 8) / 8):
##        table_x = int(pacman._x / 8)
##    if round(pacman._y / 8) == (pacman._y / 8):
##        table_y = int(pacman._y / 8)

    values = Q_table[(pacman._x - 8) / 8, (pacman._x - 8) / 8, 0]
